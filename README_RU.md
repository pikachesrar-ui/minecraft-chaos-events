...
- **Викторина** каждые 6–12 минут: 15 секунд на ответ, награды и наказания за ошибки.
...
Оператор может получить одну из четырёх книг через `/chaos book ...` и открыть её правой кнопкой мыши. В книге отображается постраничный список с переключателями `ВКЛ`/`ВЫКЛ`. Кнопка **«Сохранить»** записывает выбор в `config/chaosevents-settings.json`.

Если Chaos Events уже запущен, после сохранения текущие временные механики корректно очищаются, а внутренняя хаос-сессия запускается заново с новым набором включённого контента. Перезапуск Minecraft или сервера не требуется.

Книга свапов позволяет независимо отключить плановый обмен каждые 15–20 минут, единичный свап за первую алмазную руду и большое событие «Пространственный сдвиг».
