...
- **30 микроподлянок** для отдельных игроков с настраиваемым интервалом: мешают играть, но не рассчитаны на намеренное убийство. 28 обычных подлянок не повторяются до завершения полного цикла; скримеры могут выпадать повторно, но не два раза подряд.
...
- **Необязательная интеграция с Places**: редкие неожиданные пространственные сбои могут временно отправить одного или двух игроков в случайную безопасную локацию Places. Используются штатные портальные процедуры Level 0, Manila, Red Road, The End, Structure Bridge и Warp Tunnel. Пока игрок находится в любом измерении `places:*`, большие события и пространственные свапы персонально не действуют на него, но продолжаются для остальных. Через 3–9 минут игрок автоматически возвращается точно в точку перед входом; переход через штатную дверь Places получает такой же таймер, а самостоятельный выход отменяет возврат. Конкретные скрытые триггеры намеренно не документируются, чтобы механика оставалась сюрпризом.
...
`src/private/resources` и собирать отдельную версию командой
`.\gradlew.bat clean build -Pmod_version=0.3.4-private.1`. Подробности — в
